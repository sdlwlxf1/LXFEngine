...
template<> FORCEINLINE VectorRegister Math::Min(const VectorRegister A, const VectorRegister B)
{
	return VectorMin(A, B);
}

/** Returns the larger of the two values (operates on each component individually) */
template<> FORCEINLINE VectorRegister Math::Max(const VectorRegister A, const VectorRegister B)
{
	return VectorMax(A, B);
}

// Specialization of Lerp template that works with vector registers
template<> FORCEINLINE VectorRegister Math::Lerp(const VectorRegister& A, const VectorRegister& B, const VectorRegister& Alpha)
{
